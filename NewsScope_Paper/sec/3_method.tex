...
Both the genre and sentiment classifiers represent documents as bags of words, i.e., unordered multisets of tokens after preprocessing. When \texttt{binarize=True} (disabled by default), each token type is counted at most once per document, converting the representation to a binary presence/absence feature vector. In practice we found standard token counts to perform comparably to binarized counts on this corpus.
...
The threshold rule above is the main sentiment-specific design choice, so we omit a separate distribution plot here.
