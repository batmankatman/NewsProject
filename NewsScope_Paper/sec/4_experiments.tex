...
We present three sample articles from the corpus to illustrate the keyword and NER pipeline. For compactness, each article's top-8 TF-IDF keywords and top-8 POS-filtered keywords share one cell, with named entities shown alongside in Table~\ref{tab:samples}.

\begin{table*}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{2.8cm}p{7.7cm}p{5.3cm}@{}}
\toprule
\textbf{Article} & \textbf{Keywords} & \textbf{NER} \\
\midrule
\textit{Time Warner profits}
  & \textbf{TF-IDF:} timewarner, yearearlier, aols, aol, restate, fullyear, 639m, 111bn\\
    \textbf{POS:} timewarner, aol, profits, sales, warner, internet, profit, time
  & GPE: Ad, Google; PERSON: Time Warner, Warner; ORG: TimeWarner \\
\addlinespace
\textit{Dollar/Greenspan}
  & \textbf{TF-IDF:} greenspan, 12871, 12974, greenspans, sinche, loosening, halfpoint, deficit\\
    \textbf{POS:} dollar, deficit, months, greenspan, federal, reserve, current
  & PERSON: Alan Greenspan; ORG: Federal Reserve; FACILITY: White House \\
\addlinespace
\textit{Yukos/Rosneft}
  & \textbf{TF-IDF:} yugansk, rosneft, menatep, yukos, 479m, 540m, rosnefts, moscowbased\\
    \textbf{POS:} rosneft, yukos, yugansk, sale, unit, loan, menatep, assets
